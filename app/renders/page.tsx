...
export default async function RendersPage({ searchParams }: { searchParams: Promise<{ brand?: string }> }) {
  ...
  return (
    <main className="studio-shell"><WorkspaceHeader active="renders" /><section className="collection-page">
      <div className="collection-heading history-heading"><div><p className="kicker">Immutable history</p><h1>Reviewed output, by brand.</h1><p>Compare one brand at a time. Every asset keeps its exact content, template version, dimensions, and hash.</p></div><Link className="dark-link" href="/">Create render →</Link></div>
