...
export default async function CarouselRenderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="studio-shell"><WorkspaceHeader active="carousels" /><section className="collection-page carousel-render-page">
    ...
    <div className="history-heading collection-heading"><div><p className="kicker">Carousel render</p><h1>{render.slides[0].headline}</h1><p>The exported files are byte-for-byte promotions of the exact reviewed and approved PNGs.</p></div><a className="dark-link" href={render.zipUrl} download>Download ZIP ↓</a></div>
    <div className="carousel-render-grid">{render.artifacts.map((artifact, index) => <article className="carousel-render-slide" key={artifact.slideIndex}>
      ...
      <div><span>Slide {String(index + 1).padStart(2, "0")}</span><strong>{render.slides[index].headline}</strong><a href={artifact.assetUrl} download>Download PNG</a><small>{artifact.width} × {artifact.height}</small><code>{artifact.sha256}</code></div>
    </article>)}</div>
