...
import { Archive, Check, ChevronDown, Download, ImagePlus, RefreshCw, RotateCcw, Save, ScanSearch } from "lucide-react";
...
export function DraftWorkspace({ initialDraft, initialRevisions, initialAssets, brand, template, recentCreative }: { initialDraft: DraftRecord; initialRevisions: DraftRevisionRecord[]; initialAssets: WorkspaceAsset[]; brand: BrandRecord; template: TemplateRecord; recentCreative: RecentCreative[] }) {
  ...
  return <AppShell><section className="draft-workspace page-frame">
    ...
    <div className="workflow-action-card workflow-command-bar"><div><span className="section-overline">Next action</span><strong>{draft.status === "draft" ? "Review this revision" : draft.status === "in_review" ? "Make the human decision" : draft.status === "approved" ? "Create the final PNG" : "Export is ready"}</strong><small>{draft.status === "draft" ? "Mechanical checks verify dimensions, bounds, overflow and deterministic output." : draft.status === "in_review" ? "Inspect the artwork—not only the checks—before approving." : draft.status === "approved" ? "The exported asset stays pinned to this exact revision." : "Open Exports to download or inspect the immutable file."}</small></div><div className="workflow-primary-actions">{draft.status === "draft" && <Button disabled={busy || Boolean(draft.archivedAt) || hasChanges} onClick={review} type="button"><ScanSearch />Run review</Button>}{draft.status === "in_review" && <><Button disabled={busy} onClick={() => approve("approved")} type="button"><Check />Approve revision</Button><Button disabled={busy} onClick={() => approve("rejected")} variant="outline" type="button"><RotateCcw />Request changes</Button></>}{draft.status === "approved" && <Button disabled={busy} onClick={render} type="button"><Download />Render approved PNG</Button>}</div><div className="workflow-secondary-actions"><Button disabled={busy} onClick={archive} size="sm" variant="ghost" type="button">{draft.archivedAt ? <RotateCcw /> : <Archive />}{draft.archivedAt ? "Restore draft" : "Archive"}</Button>{image ? <span><ImagePlus /> Image attached</span> : null}</div>{hasChanges && draft.status === "draft" ? <p className="action-hint">Save the current changes before running review.</p> : null}</div>
    <div className={compositionId ? "draft-workspace-grid composition-editor-active" : "draft-workspace-grid"}><section className="draft-form panel">
      ...
      <details className="workspace-disclosure source-disclosure"><summary><span><strong>Source & media</strong><small>Prompt, image crop, and format</small></span><ChevronDown /></summary><div className="workspace-disclosure-content">
      ...
      <details className="workspace-disclosure revision-disclosure"><summary><span><strong>Revision details</strong><small>Template, provenance, and history</small></span><ChevronDown /></summary><div className="workspace-disclosure-content">
