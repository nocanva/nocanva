...
export const renderWithLocalPlaywright: MediaRenderer = async (input) => {
  ...
  try {
    ...
    if (!response?.ok()) throw new Error(`The render preview returned HTTP ${response?.status() ?? "unknown"}.`);
