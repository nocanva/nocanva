...
async function seedWorkspace(workspaceId: string) {
  ...
  const compositionStatements = (Object.entries(compositionTemplateIds) as Array<[CompositionId, string]>).flatMap(([compositionId, templateId]) => {
    ...
    return [
      ...
      ...(compositionId === "receipt" || compositionId === "product" ? [
        db.prepare("INSERT OR IGNORE INTO template_versions (id, workspace_id, template_id, version, renderer_key, config_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(`${storedTemplateId}@6`, workspaceId, storedTemplateId, 6, compositionId, JSON.stringify({ description: definition.purpose, composition: definition, designRevision: compositionId === "receipt" ? "unobscured-evidence-v6" : "transform-free-screenshot-and-visible-cta-v6" }), createdAt + 5),
      ] : []),
      ...(compositionId === "receipt" ? [
        db.prepare("INSERT OR IGNORE INTO template_versions (id, workspace_id, template_id, version, renderer_key, config_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(`${storedTemplateId}@7`, workspaceId, storedTemplateId, 7, compositionId, JSON.stringify({ description: definition.purpose, composition: definition, designRevision: "visible-semantic-conclusion-v7" }), createdAt + 6),
      ] : []),
    ];
  });
  await db.batch([
