...
test("local MCP exposes the agent-native Canvnah workflow", async () => {
  ...
  for (const tool of ["canvnah_list_brands", "canvnah_list_templates", "canvnah_create_post", "canvnah_list_posts", "canvnah_render_post", "canvnah_list_renders", "canvnah_get_render", "canvnah_rerender"]) {
    assert.match(server, new RegExp(`registerTool\\(\\"${tool}\\"`));
  }
  assert.match(client, /chromium\.launch/);
  assert.match(client, /different PNGs for the same payload/);
  assert.match(client, /only connects to a loopback Canvnah URL/);
  assert.match(fixture, /Create a Blindspot post explaining/);
