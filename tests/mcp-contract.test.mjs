...
test("local MCP exposes the agent-native Canvnah workflow", async () => {
  const [server, client, fixture, agentInstructions, packageJson] = await Promise.all([
  ...
  assert.match(server, /Autonomous local workflow/);
  ...
  assert.match(agentInstructions, /codex mcp add canvnah/);
  assert.match(agentInstructions, /claude mcp add canvnah/);
